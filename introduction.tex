...
{\ross} from \displaydate{firstross} all to March 2020. The reason why the
numbers for the visible light filters are not the same in the case of \bstar, as
with the other targets, is that the one observation each for the \texttt{i} and
the \texttt{r} filters on 11 October 2017 and one for the \texttt{r} filter on 
29 August 2019 are missing.}
\protect\label{table:proxsumm} \protect\label{table:bstarsumm} \protect\label{table:rosssumm}
\end{table}
